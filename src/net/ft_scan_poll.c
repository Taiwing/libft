	...
	struct pollfd *pfds)
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		if (pfds[i].revents & POLLIN)
		{
			if (ft_scan_recv(scans[i].scan, 0, &now) < 0)
				return (-1);
...
** scans is an array of t_pollsc strucutres specifying which scans to listen
...
**
** If events are catched they will set in the scan_control structure and
