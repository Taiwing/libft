...
// For microseconds computation
# define	MAX_TV_USEC					1000000
# define	MIN_TV_USEC					(-MAX_TV_USEC)

/*
** This will make libft compilation fail if the host system has implemented
...
int	ft_timeval_add(struct timeval *dest, const struct timeval *left,
...
int	ft_timeval_sub(struct timeval *dest, const struct timeval *left,
		const struct timeval *right)
{
	int128_t	sec = (int128_t)left->tv_sec - (int128_t)right->tv_sec;
	int128_t	usec = (int128_t)left->tv_usec - (int128_t)right->tv_usec;
...
int	ft_timeval_abs(struct timeval *dest, const struct timeval *src)
{
...
int	ft_timeval_div(struct timeval *dest, const struct timeval *src, int div)
{
	int128_t	sec = (int128_t)src->tv_sec;
	int128_t	usec = (int128_t)src->tv_usec;
	...
	if (!div)
	{
		ft_errno = E_FTERR_DIVIDE_BY_ZERO;
		return (-1);
	}
	sec /= div;
	usec /= div;
...
int	ft_timeval_mul(struct timeval *dest, const struct timeval *src, int mul)
{
	int128_t	sec = (int128_t)src->tv_sec;
	int128_t	usec = (int128_t)src->tv_usec;
